...
#write.table(x=dnase_signal_track, file="allEncodeDnasePeaks.H3K27ac_only.sorted.merged.distal.signal.normalized.withPeakNames.bed", sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)
write.table(x=dnase_signal_track, file="allEncodeDnasePeaks.H3K27ac_only.sorted.merged.distal.signal.normalized.withPeakNames.and_details.bed", sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)
#write.table(x=dnase_number_track, file="allEncodeDnasePeaks.H3K27ac_only.sorted.merged.distal.number.bed", sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)
#write.table(x=dnase_composite_track, file="allEncodeDnasePeaks.H3K27ac_only.sorted.merged.distal.signal_times_number.bed", sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)
