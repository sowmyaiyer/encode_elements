...
dnase_signal_track <- data.frame(df[,1],df[,2],df[,3],paste("re",gsub(df[,1],pattern="chr",replacement=""),".",floor((df[,2]+df[,3])/2),sep=""),normalized_score_signal, ".", df[,2],df[,3], "51,138,87", unique_cell_types_clean,number_unique_cell_types_merged,total_peaks_merged,gsub(df[,7],pattern="\\.000000", replacement=""))
